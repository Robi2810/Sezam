...
function MainScreen() {
    ...
    useEffect(()=>{kb.getAllKnowledgeBases(0,100).then(d => {
        setData(d.data.content!)
        
    }).finally(()=>{
        s.getActiveSession().then(e=> {
    ...
    return (
        <div>   
            ...
            {activeKb !== "" && concreteData !== undefined ? DetailedCard(concreteData, () => {
...
{data.map(e => Card(() => {
                        if(e.type === ShortKnowledgeBaseTypeEnum.CREDENTIALS){
                            s.createNewSession(e.id).then(()=> kb.getKnowledgeBase(e.id).then(d => setConcreteData(d.data)).finally(() => setActiveKb(e.id)))
                        } else {
                            kb.getKnowledgeBase(e.id).then(d => setConcreteData(d.data)).finally(() => setActiveKb(e.id))
                        }
